<?php
/**
 * migxcalendars
 *
 * @package migxcalendars
 * @language de
 */

$_lang['migxcal.movetothrash_ifexits'] = 'Termine in Papierkorb verschieben, falls belegt in dieser Kategorie';
$_lang['migxcal.on_double_events'] = 'Bei Terminüberschneidungen in dieser Kategorie';
$_lang['migxcal.publish'] = 'Veröffentlichen';
$_lang['migxcal.to_thrash'] = 'In den Papierkorb';
$_lang['migxcal.unpublish'] = 'Verbergen';
$_lang['migxcal.ignore'] = 'Nicht erstellen';
$_lang['migxcal.assigned_categories'] = 'Zugewiesene Kategorien';
$_lang['migxcal.category'] = 'Kategorie';
$_lang['migxcal.allday'] = 'Ganztags';
$_lang['migxcal.unpublish_dates'] = 'Termine verbergen';
$_lang['migxcal.ondates_in_assigned_cats'] = 'Bei Terminüberschneidungen in zugewiesenen Kategorien';
$_lang['migxcal.yes'] = 'Ja';
$_lang['migxcal.no'] = 'Nein';
$_lang['migxcal.inherited'] = 'Vererbt';
$_lang['migxcal.repeat_weekly'] = 'Wöchentlich wiederholen';
$_lang['migxcal.lastdate'] = 'Letzter Termin';
$_lang['migxcal.repeatings'] = 'Wiederholungen';
$_lang['migxcal.event'] = 'Veranstaltung';
$_lang['migxcal.title'] = 'Titel';
$_lang['migxcal.start'] = 'Start';
$_lang['migxcal.end'] = 'Ende';
$_lang['migxcal.start_firstdate'] = 'Start (erster Termin)'; 
$_lang['migxcal.end_firstdate'] = 'Ende (erster Termin)';
$_lang['migxcal.singledates'] = 'Einzeltermine';
$_lang['migxcal.categoryname'] = 'Name';
$_lang['migxcal.background_color'] = 'Background Color';
$_lang['migxcal.border_color'] = 'Border Color';
$_lang['migxcal.text_color'] = 'Text Color';
$_lang['migxcal.add_event'] = 'Veranstaltung erstellen';
$_lang['migxcal.events'] = 'Veranstaltungen';
$_lang['migxcal.events_cmp_desc'] = 'Hier werden die Veranstaltungen verwaltet';
$_lang['migxcal.add_category'] = 'Kategorie erstellen';
$_lang['migxcal.categories'] = 'Kategorien';
$_lang['migxcal.categories_cmp_desc'] = 'Hier werden die Kategorien verwaltet';
$_lang['migxcal.events_cmp_main_caption'] = 'Veranstaltungs Manager';
$_lang['migxcal.event_active'] = 'Veranstaltung aktiv';
$_lang['migxcal.active'] = 'Aktiv';
$_lang['migxcal.inactive'] = 'Inaktiv';
$_lang['migxcal.active_filter'] = 'Aktiv/Inaktiv';
$_lang['migxcal.check_availability'] = 'Verfügbarkeit prüfen';
$_lang['migxcal.availability'] = 'Verfügbarkeit';
$_lang['migxcal.available'] = 'Verfügbar';
$_lang['migxcal.category_filter'] = 'Kategorie Filter';
$_lang['migxcal.warn_move_repeat_2other_day'] = 'Verschieben auf einen anderen Tag wird alle Wiederholungen neu erstellen!';
$_lang['migxcal.warn_change_repeating'] = 'Wiederholen Status wurde geändert. Dies wird alle Termine dieses Containers entfernen!';
$_lang['migxcal.warn_move_repeat_2other_week'] = 'Wiederholungen können nicht auf eine andere Woche verschoben werden!';
$_lang['migxcal.warn_dragnew_repeatingdate'] = 'Es können keine Termine in einen Container mit Wiederholungen gezogen werden';
$_lang['migxcal.createon'] = 'Erstellt am';
$_lang['migxcal.search'] = 'Suche';
$_lang['migxcal.load_datecontainer'] = 'Termincontainer laden';
$_lang['migxcal.hide_published'] = 'Aktive verbergen';
$_lang['migxcal.show_unpublished'] = 'Inaktive zeigen';
$_lang['migxcal.date_containers'] = 'Termin Container';




