...
if (empty($scriptProperties['event_id'])) {
...
switch ($scriptProperties['task']) {
    case 'removeone':
        $object = $modx->getObject($classname, $scriptProperties['event_id']);
        if ($object->remove() === false) {
            return $modx->error->failure($modx->lexicon('migxcal.event_err_remove'));
        }
        break;
